  ...
  useEffect(() => {
    const fetchInventoryData = async () => {
      try {
        ...
        console.log("Danh sách nhập kho đã fetch: ", response.data);
      ...
      <table className="min-w-full bg-white rounded-lg shadow-lg">
        ...
        <tbody>
          {inventoryData.map((item, index) => (
            <tr key={item.InventoryInID}>
              <td className="px-6 py-4 whitespace-nowrap">
                {item.InventoryInDeID}
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                {item.InventoryIn.SupplierID}
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                {item.Material.MaterialName}
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                {item.AmountReceived} đ
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                {item.Quanlity} {item.Unit.UnitName}
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                {item.TotalAmount} đ
              </td>
